...
async def generate_workout(
    ...
    result = await container.generate_workout_use_case().execute(
        GenerateWorkoutCommand(user_id=user_id, **payload.model_dump())
    )
...
async def start_workout(
    workout_id: UUID, _: StartWorkoutRequestSchema
) -> APIResponseSchema[dict]:
    result = await container.start_workout_use_case().execute(
        StartWorkoutCommand(workout_id=workout_id)
    )
    return APIResponseSchema(data=asdict(result))
...
async def log_set(
    workout_id: UUID, payload: LogSetRequestSchema
) -> APIResponseSchema[dict]:
    result = await container.log_workout_set_use_case().execute(
        LogSetCommand(workout_id=workout_id, **payload.model_dump())
    )
...
async def complete_workout(
    workout_id: UUID, payload: CompleteWorkoutRequestSchema
) -> APIResponseSchema[dict]:
    result = await container.complete_workout_use_case().execute(
        CompleteWorkoutCommand(workout_id=workout_id, **payload.model_dump())
    )
    return APIResponseSchema(data=result)
